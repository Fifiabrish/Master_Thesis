...
\begin{figure}[ht]
    ...
    \end{tikzpicture}
    \caption[طرح کلی مدل محرمانگی تفاضلی موضعی]{\rl{گذار از مدل متمرکز به موضعی؛ نویز به صورت موضعی روی دستگاه کاربر اضافه می‌شود.}}
\label{fig:ldp-model}
\end{figure}
...
\begin{تعریف}[\LDP]
\label{def:alpha-ldp}
فرض کنید $\mech: \Xset \to \Zset$ یک مکانیزم تصادفی باشد که به ازای هر ورودی $x \in \Xset$، خروجی $z$ را بر اساس توزیع احتمال شرطی $Q(\cdot|x)$ تولید می‌کند.
مکانیزم \mech \ دارای ویژگی \LDP \ است، اگر برای تمام جفت ورودی‌های ممکن $x, x' \in \Xset$ و برای هر زیرمجموعه‌ی اندازه‌پذیر $\Sset \subseteq \Zset$ (در $\sigma$-جبرِ برد)، نامساوی زیر برقرار باشد:
\begin{equation}
\label{eq:ldp-def-measure}
\Pr[\mech(x) \in \Sset] \le e^{\al} \cdot \Pr[\mech(x') \in \Sset]
\end{equation}
یا به بیان دقیق‌تر ریاضی با استفاده از سوپریمم روی تمام پیشامدها:
\begin{equation}
\label{eq:ldp-def-sup}
\sup_{x, x' \in \Xset} \sup_{\Sset \in \sigma(\Zset)} \frac{Q(\Sset|x)}{Q(\Sset|x')} \le e^{\al}
\end{equation}
اگر توزیع‌های خروجی $Q(\cdot|x)$ نسبت به یک اندازه پایه $\mu$ مطلقاً پیوسته باشند (یعنی تابع چگالی $q(\cdot|x)$ وجود داشته باشد)، شرط بالا معادل است با کران‌دار بودن نسبت چگالی‌ها در تمام نقاط \cite{Duchi2013}:
\begin{equation}
\label{eq:ldp-density}
\sup_{z \in \Zset} \sup_{x, x' \in \Xset} \frac{q(z|x)}{q(z|x')} \le e^{\al}
\end{equation}
این تعریف تضمین می‌کند که برای هر خروجی مشاهده‌شده‌ی $z$، نسبت درست‌نمایی\LTRfootnote{Likelihood Ratio} اینکه ورودی $x$ بوده است یا $x'$، با مقدار $e^\al$ محدود می‌شود و در نتیجه تمایز قائل شدن بین این دو ورودی برای مهاجم دشوار می‌گردد.
\end{تعریف}
...
\begin{figure}[ht]
    ...
    \caption[مدل گرافیکی پروتکل‌های تعاملی و غیرتعاملی.]{
...
از دیدگاه آنالیز ریاضی، این فرآیند با استفاده از «هسته‌های احتمالاتی»\LTRfootnote{Probability Kernels} مدل‌سازی می‌شود. فرض کنید $Z_{1:i-1} = (Z_1, \dots, Z_{i-1})$ بردار خروجی‌های پیشین باشد که $\sigma$-میدانِ $\mathcal{F}_{i-1}$ را تولید می‌کند. مکانیزم کاربر $i$-ام، یک هسته احتمالاتی $Q_i$ است که خروجی $Z_i \in \Zset$ را مشروط بر داده‌ی خصوصی $X_i$ و تاریخچه‌ی عمومی $Z_{1:i-1}$ تولید می‌کند:
\begin{equation}
...
مکانیزم $\mech_{Lap}$ ورودی نرمال شده $x \in [-1, 1]$ را دریافت کرده و خروجی $z$ را طبق رابطه زیر تولید می‌کند:
\begin{equation}
...
حال اگر بخواهیم دمای بدن یک بیمار را که به بازه $[-1, 1]$ نرمال شده است، با بودجه $\al=1$ منتشر کنیم. اگر مقدار واقعی $x=0.5$ باشد:
\begin{itemize}
...
\textbf{تحلیل واریانس:}
اگرچه این مکانیزم اطلاعات را به شدت فشرده (۱ بیت) می‌کند، اما دوچی ثابت کرد که این روش از نظر نرخ مینی‌مکس برای تخمین میانگین بهینه است. واریانس آن برابر است با:
\begin{equation}
