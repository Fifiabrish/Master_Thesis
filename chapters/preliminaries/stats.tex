...
\label{thm:fano}
زمانی که پارامتر مورد نظر متعلق به مجموعه‌ای بزرگ‌تر ($\mathcal{V}$) باشد \\ ($M = |\mathcal{V}| > 2$)، نامساوی فانو کران پایین قوی‌تری ارائه می‌دهد:
\begin{equation}
\label{eq:fano}
\inf_{\psi} \bar{P}_{err}(\psi) \ge 1 - \frac{I\left(X^n; \mathcal{V}\right) + \log 2}{\log M}
\end{equation}
که در آن $I(X^n; \mathcal{V})$ اطلاعات متقابل بین داده‌ها و اندیس پارامتر است.

\textbf{تفسیر:} نامساوی فانو مسئله خطا را به اطلاعات متقابل $I(X^n; \mathcal{V})$ گره می‌زند. اگر داده‌ها حاوی اطلاعات کافی درباره اندیس واقعی ${v}$ نباشند (ظرفیت کانال نسبت به تعداد فرضیه‌ها $\log M$ کم باشد)، خطا اجتناب‌ناپذیر است.
\end{قضیه}
